...
const calculateStats = (
  ...
  const progressRate =
    totalPages > 0
      ? parseFloat(((readPages / totalPages) * 100).toFixed(1))
      : 0;
  const completionRate =
    total > 0 ? parseFloat(((read / total) * 100).toFixed(1)) : 0;
...
export const useBooksQuery = (usePagination = true, pageSize = 12) => {
  ...
  const infiniteQuery = useInfiniteQuery({
    queryKey: booksKeys.paginatedList(
      user?.uid || "",
      sortField,
      sortDirection,
    ),
    queryFn: async ({ pageParam }) => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            "Przełączanie na tryb niepaginowany z powodu trwałego błędu",
          );
          setUseFallback(true);
        }
  ...
  const books = useMemo(() => {
    return usePagination && !useFallback
      ? paginatedBooks
      : legacyQuery.data || [];
  ...
  const fetchNextPage = !useFallback
          ...
          (old: any) => {
            ...
            return {
              ...
              pages: old.pages.map((page: PaginatedResult<Book>) => ({
                ...
                items: page.items.map((book: Book) =>
                  book.id === bookId
                    ? { ...book, isFavorite: !currentFavorite }
                    : book,
                ),
              })),
            };
  ...
  return {
    ...
    handleBookDelete: (bookId: string) =>
      deleteBookMutation.mutateAsync(bookId),
